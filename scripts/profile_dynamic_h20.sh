...
update_bin="${repo_root}/build/update_bench"
delta_bin="${repo_root}/build/lockfree_delta_bench"
publication_bin="${repo_root}/build/publication_bench"
...
for mode in atomic warp; do
  CUDA_VISIBLE_DEVICES="${profile_gpu}" "${nsys_bin}" profile \
    ...
    --mode "${mode}" --pool-factor 16 --warmup 3 --iterations 10
  ...
  CUDA_VISIBLE_DEVICES="${profile_gpu}" "${ncu_bin}" \
    ...
    "${delta_bin}" --vertices 1024 --block-capacity 32 \
    --batch-size 100000 --input-order grouped --distribution zipf \
    --mode "${mode}" --pool-factor 16 --warmup 0 --iterations 1
done
